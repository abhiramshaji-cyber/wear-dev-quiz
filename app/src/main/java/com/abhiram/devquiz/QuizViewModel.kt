...
class QuizViewModel(app: Application) : AndroidViewModel(app) {
    ...
    // A missed question is reinserted within the next few turns, so it is retried while the
    // reason is still on screen. Never appended at the end, or a wrong answer costs a full round.
    private fun requeue(index: Int) {
        queue.add(minOf(rng.nextInt(REQUEUE_WITHIN), queue.size), index)
    }

    private fun fill() {
        val order = Questions.all.indices.shuffled(rng).toMutableList()
        val missed = store.missed()
        Questions.all.indices
